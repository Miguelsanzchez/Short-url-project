...
(async () => {
    ...
    if (!migrationsDir.length) throw Error("No migrations files found");

    const connection = await db;

    await connection.query(createControl);
    const migratedId = await connection.query(selectMigratedIds);
    const migratedList = new Set(migratedId.rows.map(({ id }) => id));
